...
def data_read(data_path,category='Original'):
	labels = []
	images = []
	...
	for label in os.listdir(data_path):
		...
		for image in os.listdir(label_path):
			img = cv2.imread(os.path.join(label_path,image),0)
			if(img is not None):
				resized_img = image_resize(img)
				images.append(np.reshape(resized_img,(np.shape(resized_img)[0],np.shape(resized_img)[1],1)))
				labels.append(label)
	return labels, images

labels, images = data_read(r'C:\Users\Abhinav\Desktop\Work\ASL Translation')
